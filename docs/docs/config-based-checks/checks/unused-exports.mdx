...
description: Detect and remove exports that are not reachable from any project entry points, helping to clean up dead code and reduce bundle size.
---
...
If you see reported false positives, follow [Orphan files and unused exports in shared packages](../../troubleshooting/orphan-files-and-unused-exports-in-shared-packages.mdx) guide.



## Configuration

Below is an example of how `unusedExportsDetection` fits within the `workspaces` array. It demonstrates defining general production and development entry points at the **workspace level**.
...
*Tip: To suppress unused-export findings for a leftover-but-committed file, add it to the workspace-level [`ignoreEntryPoints`](../entry-points-definition.mdx#ignoring-leftover-entry-points). Files matching `ignoreEntryPoints` have all their exports treated as used.*
