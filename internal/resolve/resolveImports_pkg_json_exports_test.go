...
func TestPackageJsonExportsDifferentSpecificity(t *testing.T) {
	...
	if exactMatchImport == nil {
		...
		if exactMatchImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected exact match import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(exactMatchImport.ResolvedType))
		}
	...
	if wildcardImport == nil {
		...
		if wildcardImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected wildcard import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(wildcardImport.ResolvedType))
		}
...
func TestPackageJsonExportsConditionalExports(t *testing.T) {
	...
	if conditionalImport == nil {
		...
		if conditionalImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected conditional import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(conditionalImport.ResolvedType))
		}
	...
	if conditionalImportProd == nil {
		...
		if conditionalImportProd.ResolvedType != MonorepoModule {
			t.Errorf("Expected conditional import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(conditionalImportProd.ResolvedType))
		}
	...
	if conditionalImportDefault == nil {
		...
		if conditionalImportDefault.ResolvedType != MonorepoModule {
			t.Errorf("Expected conditional import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(conditionalImportDefault.ResolvedType))
		}
...
func TestPackageJsonExportsRootWildcard(t *testing.T) {
	...
	if rootWildcardImport == nil {
		...
		if rootWildcardImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected root wildcard import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(rootWildcardImport.ResolvedType))
		}
...
func TestPackageJsonExportsDirectorySwap(t *testing.T) {
	...
	if directorySwapImport == nil {
		...
		if directorySwapImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected directory swap import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(directorySwapImport.ResolvedType))
		}
...
func TestPackageJsonExportsDeepNestedConditionalExports(t *testing.T) {
	...
	if deepDevNodeImport == nil {
		...
		if deepDevNodeImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected deep dev+node import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(deepDevNodeImport.ResolvedType))
		}
	...
	if deepDevDefaultImport == nil {
		...
		if deepDevDefaultImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected deep dev+default import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(deepDevDefaultImport.ResolvedType))
		}
	...
	if deepProdBrowserImport == nil {
		...
		if deepProdBrowserImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected deep prod+browser import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(deepProdBrowserImport.ResolvedType))
		}
	...
	if deepProdDefaultImport == nil {
		...
		if deepProdDefaultImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected deep prod+default import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(deepProdDefaultImport.ResolvedType))
		}
	...
	if deepDefaultImport == nil {
		...
		if deepDefaultImport.ResolvedType != MonorepoModule {
			t.Errorf("Expected deep default import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(deepDefaultImport.ResolvedType))
		}
...
func TestPackageJsonExportsResolvesSelfImportInWorkspacePackage(t *testing.T) {
	...
	if selfImport.ResolvedType != MonorepoModule {
		t.Fatalf("Expected self import type to be MonorepoModule, got '%s'", model.ResolvedImportTypeToString(selfImport.ResolvedType))
	}
...
func findImportByRequest(imports []model.MinimalDependency, request string) *model.MinimalDependency {
...
func TestPackageJsonExportsBaseUrlWildcard(t *testing.T) {
	...
	if imports[0].ID != expectedPath || imports[0].ResolvedType != MonorepoModule {
		t.Errorf("Expected %s with MonorepoModule type, got '%s' with type %s", expectedPath, imports[0].ID, model.ResolvedImportTypeToString(imports[0].ResolvedType))
	}
}
...
func TestPackageJsonExportsNoMain(t *testing.T) {
	...
	if imports0[0].ID != expectedIndex || imports0[0].ResolvedType != MonorepoModule {
		t.Errorf("Expected %s with MonorepoModule type, got '%s' with type %s", expectedIndex, imports0[0].ID, model.ResolvedImportTypeToString(imports0[0].ResolvedType))
	}
	...
	if imports[0].ID != expectedIndex || imports[0].ResolvedType != MonorepoModule {
		t.Errorf("Expected %s with MonorepoModule type, got '%s' with type %s", expectedIndex, imports[0].ID, model.ResolvedImportTypeToString(imports[0].ResolvedType))
	}
	...
	if imports2[0].ID != expectedUtils || imports2[0].ResolvedType != MonorepoModule {
		t.Errorf("Expected %s with MonorepoModule type, got '%s' with type %s", expectedUtils, imports2[0].ID, model.ResolvedImportTypeToString(imports2[0].ResolvedType))
	}
	...
	if imports3[0].ID != expectedUtilsIndex || imports3[0].ResolvedType != MonorepoModule {
		t.Errorf("Expected %s with MonorepoModule type, got '%s' with type %s", expectedUtilsIndex, imports3[0].ID, model.ResolvedImportTypeToString(imports3[0].ResolvedType))
	}
	...
	if imports4[0].ID != expectedLibIndex || imports4[0].ResolvedType != MonorepoModule {
		t.Errorf("Expected %s with MonorepoModule type, got '%s' with type %s", expectedLibIndex, imports4[0].ID, model.ResolvedImportTypeToString(imports4[0].ResolvedType))
	}
}
