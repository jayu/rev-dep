...
export const toolkitTabs: ToolkitTab[] = [
  ...
  },
  {
    ...
    question: 'What has been copy-pasted?',
    explanation:
      'Copy-pasted code blocks and JSX elements. Each finding is a unit you can extract, not a line range you have to untangle first. Add --blind-identifiers and a renamed copy counts too.',
    command: 'rev-dep duplicated-code',
    docs: '/docs/exploratory-toolkit/duplicated-code',
    lines: [
      { text: '$ rev-dep duplicated-code', tone: 'prompt' },
      { text: '' },
      { text: 'Found 1 duplicated pattern in 3 files (3 occurrences, exact comparison)' },
      { text: '' },
      { text: '#1  3 duplicates in 3 files  -  code block, 9 lines, depth 2', tone: 'accent' },
      { text: '    src/services/adminApi.ts:3:53' },
      { text: '    src/services/teamApi.ts:6:84' },
      { text: '    src/services/userApi.ts:4:84' },
      { text: '' },
      { text: '  | {',  },
      { text: '  |   const request = buildRequest(endpoint, payload);',  },
      { text: '  |   const response = await transport.send(request);',  },
      { text: '  |   if (!response.ok) throw new TransportError(response);',  },
