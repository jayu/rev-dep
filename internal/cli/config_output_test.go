...
func TestPrintRestrictedImportsResolveHint_UsesViolationIgnoreTypeFlag(t *testing.T) {
	...
	// Single quotes, not double: inside double quotes the shell still expands $ and executes
	// backticks, so a pattern containing either would not survive being pasted.
