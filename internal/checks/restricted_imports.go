...
func FindRestrictedImports(
	...
	entryPointMatchers := globutil.CreateGlobMatchers(opts.EntryPoints, rulePath)
	entryPoints := []string{}
	entryPointSet := map[string]bool{}

	for filePath := range graphTree {
		if globutil.MatchesAnyGlobMatcher(filePath, entryPointMatchers, false) {
	...
	graph := graph.BuildDepsGraphForMultiple(graphTree, entryPoints, nil, false, opts.IgnoreTypeImports)
	...
	for _, filePath := range sortedFilePaths {
		...
		if len(denyFileMatchers) > 0 &&
			...
			globutil.MatchesAnyGlobMatcher(filePath, denyFileMatchers, false) &&
...
func matchesIgnoredPattern(candidate string, ignoreMatchers []globutil.GlobMatcher) bool {
	return len(ignoreMatchers) > 0 && globutil.MatchesAnyGlobMatcher(candidate, ignoreMatchers, false)
}

func filterTreeByGraphExclude(tree MinimalDependencyTree, excludeMatchers []globutil.GlobMatcher) MinimalDependencyTree {
	...
	for filePath, deps := range tree {
		if globutil.MatchesAnyGlobMatcher(filePath, excludeMatchers, false) {
			continue
		}

		filteredDeps := make([]MinimalDependency, 0, len(deps))
		for _, dep := range deps {
			if dep.ID != "" && globutil.MatchesAnyGlobMatcher(dep.ID, excludeMatchers, false) {
				continue
			}
