...
func configProcessorFixture(t *testing.T) string {
	t.Helper()
	path, err := testutil.FixturePath("configProcessorProject")
	if err != nil {
...
func TestConfigProcessor_UnusedNodeModules(t *testing.T) {
	...
	for _, ruleResult := range result.RuleResults {
		for _, module := range ruleResult.UnusedNodeModules {
			foundUnusedModules = append(foundUnusedModules, module.ModuleName)
		}
	}
...
func TestFilterFilesForRule_FollowMonorepoPackagesSelective(t *testing.T) {
	...
	resolverManager := resolve.NewResolverManagerWithMonorepoContext(&monorepo.MonorepoContext{
	...
	ruleFiles, ruleTree := filterFilesForRule(
		...
		rulePath,
		cwd,
		model.FollowMonorepoPackagesValue{Packages: map[string]bool{"@scope/allowed": true}},
		resolverManager,
	)
