...
Read more in [Orphan files and unused exports in shared packages](../../troubleshooting/orphan-files-and-unused-exports-in-shared-packages.mdx).


## Configuration

Below is an example of how `orphanFilesDetection` fits within the `workspaces` array. It demonstrates defining general production and development entry points at the **workspace level**.
  ...
  "workspaces": [
    {
      ...
      "orphanFilesDetection": {
        ...
        "autofix": true,
        // overrides the workspace-level entry points settings for this check
...
*Note: While the workspace-level `prodEntryPoints` and `devEntryPoints` provide a baseline, you can use `validEntryPoints` within the check configuration to explicitly define the roots for your orphan analysis.*

*Tip: To keep a leftover-but-committed file from being reported as orphan, add it to the workspace-level [`ignoreEntryPoints`](../entry-points-definition.mdx#ignoring-leftover-entry-points) instead of deleting it or padding `validEntryPoints`.*
