...
type fileLocationResolver struct {
	cwd       string
	tree      model.MinimalDependencyTree
...
type fileLocationIndex struct {
	byRequest    map[string][]parser.SimpleLocation
	byModuleName map[string][]parser.SimpleLocation
	byExportName map[string][]parser.SimpleLocation
}

func newFileLocationResolver(cwd string, tree model.MinimalDependencyTree) *fileLocationResolver {
...
func (r *fileLocationResolver) getDeps(filePath string) ([]model.MinimalDependency, bool) {
	if filePath == "" {
		return nil, false
	}
	...
	if !filepath.IsAbs(filePath) {
		abs := pathutil.NormalizePathForInternal(pathutil.JoinWithCwd(r.cwd, filePath))
...
func (r *fileLocationResolver) getFileCode(filePath string) ([]byte, bool) {
	...
	if !filepath.IsAbs(absPath) {
		absPath = pathutil.JoinWithCwd(r.cwd, absPath)
	}
...
func (r *fileLocationResolver) nextLocation(filePath string, kind string, key string) *parser.SimpleLocation {
	...
	var list []parser.SimpleLocation
	switch kind {
	case "request":
...
func (r *fileLocationResolver) packageJsonLocation(filePath string, moduleName string) *parser.SimpleLocation {
...
func (r *fileLocationResolver) getIndex(filePath string) (*fileLocationIndex, bool) {
	...
	index := &fileLocationIndex{
		byRequest:    make(map[string][]parser.SimpleLocation),
		byModuleName: make(map[string][]parser.SimpleLocation),
		byExportName: make(map[string][]parser.SimpleLocation),
	}
...
func keywordLocation(code []byte, kw model.KeywordInfo) parser.SimpleLocation {
	return parser.LocationFromOffsets(code, kw.Start, kw.End)
}

func primaryFromDependency(code []byte, dep model.MinimalDependency) parser.SimpleLocation {
	imp := model.Import{
		Request:            dep.Request,
	...
	return parser.ResolvePrimaryLocation(code, imp)
}
...
func locationFieldsFromSimple(loc *parser.SimpleLocation) jsonLocationFields {
	if loc == nil {
...
func findPackageJsonDependencyLocation(code []byte, moduleName string) *parser.SimpleLocation {
		...
		case '"':
			...
			if len(stack) > 0 && stack[len(stack)-1].kind == '{' && stack[len(stack)-1].expectingKey {
				...
				if stack[len(stack)-1].isDepsObject && lastKey == moduleName {
					loc := parser.LocationFromOffsets(code, uint32(lastKeyStart), uint32(lastKeyEnd))
					return &loc
				}
			}
			i = next
