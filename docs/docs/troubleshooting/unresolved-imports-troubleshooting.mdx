...
Config workspaces follow all workspace packages **by default**, but the exploratory CLI commands follow **none** unless told to. If you investigate with `rev-dep unresolved`, `resolve`, `files`, etc. in a monorepo, pass `--follow-monorepo-packages` - otherwise cross-package imports look unresolved even when they are fine.
...
**Fix:** declare the alias in `tsconfig.json` paths or a package map, and make sure rev-dep reads the right config (`--tsconfig-json` / `--package-json`, or the workspace's `path`). 
...
**Fix:** enable following on the workspace/command, or add the package to the follow list.
