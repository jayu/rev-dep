...
description: Detect and prevent dependency cycles (circular imports) between files, ensuring a clean and maintainable dependency graph.
---
...
Example `circularImportsDetection` configuration:
...
Cycle detection uses Strongly Connected Components (SCC). It reports one cycle per group of mutually dependent files, so a tangle of files gives you one finding instead of many overlapping ones.
