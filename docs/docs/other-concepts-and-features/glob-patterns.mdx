...
Wherever rev-dep takes a file path or pattern - entry points, `protectedFiles`, `denyFiles`, `ignoreMatches`, `ignoreFiles`, `graphExclude`, and so on - the value is matched as a **glob pattern**.
