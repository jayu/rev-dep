...
Entry-point patterns are matched as [glob patterns](../other-concepts-and-features/glob-patterns.mdx), so `[`, `]`, `{`, and `}` are **special characters**. A literal path that contains them - such as a Next.js dynamic route `pages/admin/[clientId]/edit.tsx` - is read as a glob (`[clientId]` becomes a character class) and matches **nothing**, so the entry point is silently ignored.

**If an entry-point path contains square or curly brackets, escape them** - for example `"pages/admin/\\[clientId\\]/edit.tsx"`. See [Glob patterns](../other-concepts-and-features/glob-patterns.mdx) for the full explanation and more examples.
