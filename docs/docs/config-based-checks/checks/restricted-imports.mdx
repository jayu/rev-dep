---
title: Forbidden or Restricted Imports
description: Prevent the accidental import of restricted modules or files, such as preventing server-side code from leaking into client-side bundles.
---
...
- [`restrictedImporters`](config-based-checks/checks/restricted-importers.mdx) - the dual: whitelist which entry points may transitively reach a target.
- [`restrictedDirectImporters`](config-based-checks/checks/restricted-direct-importers.mdx) - constrain which files may **directly** import a target (non-transitive, graph-free).
