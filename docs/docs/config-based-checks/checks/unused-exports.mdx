...
*Tip: To suppress unused-export findings for a leftover-but-committed file, add it to the rule-level [`ignoreEntryPoints`](../entry-points-definition.mdx#ignoring-leftover-entry-points). Files matching `ignoreEntryPoints` have all their exports treated as used.*
