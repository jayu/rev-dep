...
## Glob patterns now follow `.gitignore` rules
...
They do **not** affect globs matched against non-path values, which are unchanged: `ignoreImports` (e.g. `@internal/*`), `ignoreExports` (e.g. `Internal*`), and `restrictedImportsDetection.denyModules` (e.g. `lodash/*`).
...
**You cannot re-include a file inside an excluded directory.**

```jsonc
["src/**", "!src/vendor/**"]   // src/vendor/lib.ts stays matched
```

`src/**` also matches the directory `src/vendor`, so the exception cannot apply. Exclude in a way that does not match the directory:

```jsonc
["src/*", "!src/vendor"]       // src/vendor/lib.ts is re-included
```
