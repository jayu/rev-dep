...
Below is an example of how `moduleBoundaries` fits within the `workspaces` array. The first workspace uses `deny` as a blocklist; the second uses `allow` as a whitelist:
...
- [`restrictedImports`](config-based-checks/checks/restricted-imports.mdx) - deny specific files/modules reachable from selected entry points.
- [`restrictedImporters`](config-based-checks/checks/restricted-importers.mdx) - whitelist which entry points may transitively reach a target.
- [`restrictedDirectImporters`](config-based-checks/checks/restricted-direct-importers.mdx) - constrain which files may directly import a target (non-transitive).
