...
Run at the **monorepo root**, this creates a root workspace plus one workspace per discovered workspace package. 
...
Entry points are the roots of reachability - the foundation for orphan files, unused exports, and dev-dependency checks. Define them once per workspace:
...
rev-dep checks dependency **declarations** against the workspace's `package.json` (it never scans `node_modules/`). The rule of thumb: **whoever compiles the code declares its dependencies**. Enable these on each package that owns a `package.json`.
