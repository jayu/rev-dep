...
Any file outside `utils/configs/**` that directly imports `utils/configs/serverConfig/index.ts` is reported.
...
- `ignoreMatches` (array of strings, optional): Importer/target patterns to drop from the results.
