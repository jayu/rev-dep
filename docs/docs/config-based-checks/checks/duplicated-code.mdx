...
The snapshot is versioned by its `schemaVersion` field and has a published [JSON schema](https://github.com/jayu/rev-dep/blob/master/output-schema/duplicated-code-snapshot-1.0.schema.json). A file written at any other schema version is rejected rather than half-trusted, since the settings it records are applied to the run - re-create it with `--update-snapshot`.
