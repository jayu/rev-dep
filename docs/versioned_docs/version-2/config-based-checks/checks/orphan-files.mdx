...
*Tip: To keep a leftover-but-committed file from being reported as orphan, add it to the rule-level [`ignoreEntryPoints`](../entry-points-definition.mdx#ignoring-leftover-entry-points) instead of deleting it or padding `validEntryPoints`.*
