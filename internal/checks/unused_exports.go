...
type UnusedExport struct {
	...
	ExportName string
	IsType     bool
	Fix        *sourceedit.Change // Pre-computed fix, nil if not auto-fixable
}

type UnusedExportsFilterOptions struct {
...
func FindUnusedExports(
	...
	for _, file := range ruleFiles {
		if globutil.MatchesAnyGlobMatcher(file, graphExcludeGlobs, false) {
			continue
		}
...
func FilterUnusedExports(unusedExports []UnusedExport, opts *UnusedExportsFilterOptions, cwd string) []UnusedExport {
	if opts == nil {
		return unusedExports
	}

	ignoreMatcher := globutil.NewFileValueIgnoreMatcher(opts.Ignore, opts.IgnoreFiles, opts.IgnoreExports, cwd)
...
func computeFullStatementRemoval(filePath string, dep *MinimalDependency) *sourceedit.Change {
...
func computeDefaultExportFix(filePath string, dep *MinimalDependency) *sourceedit.Change {
	source, err := os.ReadFile(filePath)
	if err != nil {
		return nil
	}

	declStart := parser.SkipSpacesAndComments(source, int(dep.ExportDeclStart))
...
func computeSurgicalBraceFix(filePath string, dep *MinimalDependency, unusedExports []exportEntry) *sourceedit.Change {
	source, err := os.ReadFile(filePath)
	if err != nil {
		return nil
	}
