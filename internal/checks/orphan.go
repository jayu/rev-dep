...
func FindOrphanFiles(
	...
	excludeGlobs := globutil.CreateGlobMatchers(graphExclude, cwd)
	...
	for filePath, fileDeps := range minimalTree {
		...
		if globutil.MatchesAnyGlobMatcher(filePath, excludeGlobs, false) {
			continue
		}
		...
		for _, dependency := range fileDeps {
			...
			if _, exists := minimalTree[depPath]; exists && !globutil.MatchesAnyGlobMatcher(depPath, excludeGlobs, false) {
	...
	for filePath := range minimalTree {
		...
		isEntryPoint := len(entryPointGlobs) > 0 && globutil.MatchesAnyGlobMatcher(filePath, entryPointGlobs, false)
