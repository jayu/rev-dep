...
func TestGetNodeModulesFromPkg(t *testing.T) {
	...
	deps, devDeps := module.GetNodeModulesFromPkgJson([]byte(pkgJson))
...
func TestResolveNodeModules(t *testing.T) {
	...
	for idx, isOk := range results {
		if !isOk {
			t.Errorf("Module %d not resolved correctly. ID: '%s', Type: %s", idx, nodeModulesImports[idx].ID, model.ResolvedImportTypeToString(nodeModulesImports[idx].ResolvedType))
		}
	}
}
...
func createTestResolverManager(cwd string, files []string, moduleSuffixes []string) *ResolverManager {
	...
	rm := NewResolverManagerForTests(nil, nil, NewModuleResolverForTests(tsConfig, cwd))
	rm.SetFilesAndExtensions(filesAndExtensions)
	return rm
}
