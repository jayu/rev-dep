...
- `!` - a leading `!` **negates** the pattern (see below)

## Negated patterns (`!`)

A pattern that begins with `!` is a **negation**: it carves an exception out of the other patterns in the same list. Within a single list, a path matches when it matches **at least one** normal (positive) pattern **and none** of the negated ones. This mirrors `.gitignore`, where `!` re-includes something a previous rule would have matched.

```jsonc
{
  "files": [
    "src/**",          // everything under src…
    "!src/vendor/**"   // …except the vendored code
  ]
}
```
