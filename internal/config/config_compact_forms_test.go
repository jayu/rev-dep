...
func TestParseConfig_CompactDetectorForms(t *testing.T) {
	t.Run("boolean true enables detector", func(t *testing.T) {
		cfg, err := ParseConfig([]byte(`{
			"configVersion": "1.11",
			"workspaces": [{"path": ".", "unresolvedImportsDetection": true}]
		}`))
		if err != nil {
	...
	t.Run("boolean false disables detector", func(t *testing.T) {
		cfg, err := ParseConfig([]byte(`{
			"configVersion": "1.11",
			"workspaces": [{"path": ".", "circularImportsDetection": false}]
		}`))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("object without enabled is treated as enabled", func(t *testing.T) {
		cfg, err := ParseConfig([]byte(`{
			"configVersion": "1.11",
			"workspaces": [{"path": ".", "restrictedImportsDetection": {"entryPoints": ["src/index.ts"], "denyFiles": ["src/secret.ts"]}}]
		}`))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("object with explicit enabled false and options is disabled", func(t *testing.T) {
		cfg, err := ParseConfig([]byte(`{
			"configVersion": "1.11",
			"workspaces": [{"path": ".", "orphanFilesDetection": {"enabled": false, "validEntryPoints": ["src/index.ts"]}}]
		}`))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("array mixes booleans and objects", func(t *testing.T) {
		cfg, err := ParseConfig([]byte(`{
			"configVersion": "1.11",
			"workspaces": [{"path": ".", "unusedNodeModulesDetection": [true, {"excludeModules": ["typescript"]}]}]
		}`))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("array item false disables that detection", func(t *testing.T) {
		cfg, err := ParseConfig([]byte(`{
			"configVersion": "1.11",
			"workspaces": [{"path": ".", "unusedNodeModulesDetection": [false, {"excludeModules": ["typescript"]}]}]
		}`))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
