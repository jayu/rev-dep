...
type ImportConventionViolation struct {
	...
	Fix           *sourceedit.Change // Optional autofix change
}
...
func compileDomains(domains []rules.ImportConventionDomain, compiledAliases []CompiledAlias, cwd string) []CompiledDomain {
	/**
	...
	*/
	...
	for _, domain := range domains {
		...
		if strings.HasSuffix(domain.Path, "*") {
			...
			directories, err := os.ReadDir(pathutil.DenormalizePathForOS(baseDir))
			...
			for _, directory := range directories {
				if directory.IsDir() && dirNameMatcher.Match(directory.Name()) {
					absolutePath := pathutil.StandardiseDirPath(pathutil.NormalizePathForInternal(filepath.Join(baseDir, directory.Name())))
			...
			absolutePath := pathutil.StandardiseDirPathInternal(pathutil.NormalizePathForInternal(filepath.Join(cwd, domain.Path)))
			aliasReplacement := ""
			aliasPathPrefix := ""
			...
			if domain.Alias != "" {
				aliasReplacement = pathutil.StandardiseDirPathInternal(domain.Alias) // user defined alias might not have trailing `/`
...
func compileAliases(tsConfigParsed *resolve.TsConfigParsed, packageJsonImports *resolve.PackageJsonImports, cwd string) []CompiledAlias {
	...
	for _, aliasMapping := range aliasMappings {
		absoluteTarget := pathutil.NormalizePathForInternal(filepath.Join(cwd, aliasMapping.Target)) // on Windows filepath.Join will add wrong slash
...
func adjustImportPathStyle(newPath, originalRequest string) string {
	...
	hasExtension := false
	for _, ext := range resolve.SourceExtensions {
...
func CheckImportConventionsFromTree(
	...
	files []string,
	parsedRules []rules.ImportConventionRule,
	resolver *resolve.ModuleResolver,
	cwd string,
	autofix bool,
