---
title: Find Undeclared Dependencies
description: Detect imports of packages that are missing from your `package.json` to prevent runtime failures and broken builds in CI/CD or production.
---

# Missing node modules

`missingNodeModulesDetection`This check helps to detect and prevent usage of undeclared dependencies by identifying imports of packages that are not declared in your `package.json`.
...
Below is an example of how `missingNodeModulesDetection` fits within the `workspaces` array in your configuration:
