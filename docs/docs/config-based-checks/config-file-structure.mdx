...
- [`nodeModulesResolution`](other-concepts-and-features/following-monorepo-packages.mdx#node-modules-resolution): which `package.json` each third-party import is validated against for the `missingNodeModules`, `unusedNodeModules`, and `unresolvedImports` checks. `"entry-package"` (default) validates every import against the rule's entry `package.json`; `"nearest-package"` validates each import against the `package.json` that owns the importing file (correct for pnpm's default layout, where each package resolves only the dependencies it declares). Applies to all rules.
...
```json
{
  ...
  "$schema": "https://github.com/jayu/rev-dep/blob/master/config-schema/1.9.schema.json?raw=true", 
