...
description: Detect when packages listed in `devDependencies` are imported by production code, preventing runtime failures and deployment errors.
---
...
Which `package.json`'s `devDependencies` are checked is determined by the top-level [`nodeModulesResolution`](../../other-concepts-and-features/node-modules-resolution.mdx) setting.
...
While it is best practice to define `prodEntryPoints` and `devEntry entryPoints` at the **workspace level** (to ensure consistency across all checks in that workspace), you can override them at the **check level** if a specific workspace requires a more granular or different set of entry points.
...
```
*In the example above, the check specifically targets `src/pages/**/*.tsx` as entry points, overriding the workspace-level defaults for this specific check.*

## Options

- `enabled` (boolean): Whether to enable dev-dependency usage detection.
- `prodEntryPoints` (array of strings): Production entry point patterns to trace dependencies from. If not provided, the workspace-level `prodEntryPoints` are used.
