...
func fixturePath(t testing.TB, name string) string {
	t.Helper()
	path, err := testutil.FixturePath(name)
	if err != nil {
...
func BenchmarkParseImportsWithTypes(b *testing.B) {
	fixtureContent, _ := os.ReadFile(fixturePath(b, "parseImports.ts"))
...
func BenchmarkParseImportsWithTypes600Loc(b *testing.B) {
	fixtureContent, _ := os.ReadFile(fixturePath(b, "parseImports600Loc.ts"))
	ignoreTypeImports := true
...
`
	...
	results, errCount := ParseImportsFromFiles([]string{pathutil.NormalizePathForInternal(vuePath)}, false, ParseModeBasic)
	if errCount != 0 {
...
`
	...
	results, errCount := ParseImportsFromFiles([]string{pathutil.NormalizePathForInternal(sveltePath)}, false, ParseModeDetailed)
	if errCount != 0 {
		t.Fatalf("Expected errCount=0, got %d", errCount)
	}
	if len(results) != 1 {
