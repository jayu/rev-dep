...
func captureJSONOutput(t *testing.T, result *config.ConfigProcessingResult, cwd string) jsonOutput {
	t.Helper()
...
func TestJSONOutput_AllChecksPassing(t *testing.T) {
	...
	if output.Version != "1.1" {
		t.Errorf("expected version '1.1', got '%s'", output.Version)
	}
