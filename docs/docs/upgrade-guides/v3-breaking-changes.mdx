...
:::tip
Run `rev-dep config migrate` to apply the safe changes automatically (renames `rules` → `workspaces`, bumps `configVersion`, drops the removed `algorithm` option) and get a list of the glob patterns and behavior changes you still need to review by hand. It edits the file in place — review the change with git.
:::
