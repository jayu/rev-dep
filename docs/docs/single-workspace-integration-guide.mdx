...
The first check that needs nothing but correct resolution - no entry points, no tuning. `config init` already enables it, so it works as generated. A high-signal, low-effort first architectural win. `ignoreTypeImports` lets you catch new runtime cycles while tolerating existing type-only ones.
...
Also independent of entry points, and unlike every other check `config init` puts this one on the
...
finds repeated code blocks and JSX elements that you can extract into shared implementations. Refactor them straight away **or accept the current state and move on.** This is the usual choice on an existing codebase. Anyways, generate the the snapshot file to establish the baseline for future runs:
