...
var binaryWordMatchersCache sync.Map // string -> *regexp.Regexp

func binaryWordMatcher(binary string) *regexp.Regexp {
	if cached, has := binaryWordMatchersCache.Load(binary); has {
		return cached.(*regexp.Regexp)
	}
	// QuoteMeta is required: binary names contain "@", "/", "." and "-".
	matcher := regexp.MustCompile(`(^|[^0-9A-Za-z_.-])` + regexp.QuoteMeta(binary) + `($|[^0-9A-Za-z_.-])`)
	binaryWordMatchersCache.Store(binary, matcher)
	return matcher
}

func binaryNameForModule(moduleName string) string {
...
// parseBinField turns a package.json "bin" value into the list of binary names it declares.
// A string value means a single binary named after the package, without its scope; an
// object maps binary names to paths and its keys are used verbatim, scope or not. Names are
// sorted so the result does not depend on map iteration order. Anything else (absent, null,
// malformed) declares no binaries.
func parseBinField(raw json.RawMessage, moduleName string) []string {
	...
	case '"':
		...
		return []string{binaryNameForModule(moduleName)}
	case '{':
		var multiple map[string]string
		if err := json.Unmarshal(trimmed, &multiple); err != nil || len(multiple) == 0 {
			return nil
		}
		names := make([]string, 0, len(multiple))
		for binaryName := range multiple {
...
// FindNodeModuleBinaries maps each declared dependency to the binaries it provides, by
// reading the "bin" field of its package.json in the nearest node_modules that has it.
	...
	for nodeModule := range nodeModules {
		for _, nodeModuleDir := range nodeModuleDirs {
			...
			if fileInfoErr == nil && !fileInfo.IsDir() {
				...
				// Decode "bin" only. It is either a string (one binary, named after the
				// package) or an object of name -> path. Pulling it out as RawMessage means
				// one pass over the bytes and no decoding of the dependency trees and other
				// large fields nobody here reads.
				var pkgJson struct {
					Bin json.RawMessage `json:"bin"`
				}
				if err := json.Unmarshal(fileContent, &pkgJson); err != nil {
					continue
				}

				binaries := parseBinField(pkgJson.Bin, nodeModule)
				if len(binaries) == 0 {
					continue
				}

				result[nodeModule] = append(result[nodeModule], binaries...)
				// If a directory closer to cwd has this node module with binaries, node will
				// resolve that one, so stop looking further up.
				break
			}
		}
	}
...
func NodeModulesCmd(
	...
	cwd := pathutil.StandardiseDirPath(inputCwd)
	excludeFiles := []string{}
	absolutePathToEntryPoints, discoveredFiles := resolve.ResolveEntryPointsFromPatterns(cwd, entryPoints, excludeFiles, nil)
	...
	// nearest-package classifies each file against its own package.json (SelfResolver);
	// entry-package classifies everything against the cwd package.json (CwdResolver, current default).
	nodeModulesMatchingStrategy := resolve.NodeModulesMatchingStrategyCwdResolver
	if nearestPackage {
		nodeModulesMatchingStrategy = resolve.NodeModulesMatchingStrategySelfResolver
	}

	minimalTree, _, resolverManager := resolve.GetMinimalDepsTreeForCwd(cwd, ignoreType, excludeFiles, nil, upfrontFilesList, packageJson, tsconfigJson, conditionNames, followMonorepoPackages, nil, nodeModulesMatchingStrategy)
	...
	// In nearest-package mode, "unused" (variant A) only counts a cwd dependency as used when one
	// of the cwd package's OWN files imports it. Build that file set from the tree; nil = all files.
	var entryOwnedFiles map[string]bool
	if nearestPackage && resolverForCwd != nil {
		entryOwnedFiles = make(map[string]bool, len(minimalTree))
		for filePath := range minimalTree {
			if resolverManager.GetResolverForFile(filePath) == resolverForCwd {
				entryOwnedFiles[filePath] = true
			}
		}
	}

	// When includeDevDepsFromRoot is set, the monorepo root devDependencies are treated as
	// available to package code for the missing check, mirroring the config option of the same
	// name. It is intentionally scoped to the missing check (not unused).
	...
	if listMissing {
		missingResults := GetMissingNodeModulesFromTree(minimalTree, modulesToInclude, modulesToExclude, cwdNodeModules, rootDevDependencies, nearestPackage)
	...
	usedNodeModules := GetUsedNodeModulesFromTree(minimalTree, cwdNodeModules, cwd, pkgJsonFieldsWithBinaries, filesWithBinaries, filesWithModules, packageJson, tsconfigJson, nil)
...
func GetUsedNodeModulesFromTree(
	...
	packageJson string,
	tsconfigJson string,
	entryOwnedFiles map[string]bool,
	...
	for filePath, fileDeps := range minimalTree {
		// nearest-package "unused" (variant A): a dependency declared by the entry package
		// counts as used only when one of the entry's OWN files imports it. When entryOwnedFiles
		// is non-nil, restrict the usage scan to those files. nil means "all files" (entry-package).
		if entryOwnedFiles != nil && !entryOwnedFiles[filePath] {
			continue
		}
		for _, dependency := range fileDeps {
			if dependency.ResolvedType == NodeModule {
				depId := module.GetNodeModuleName(dependency.Request)
	...
	for _, filePath := range filesWithBinaries {
		absoluteFilePath := pathutil.JoinWithCwd(cwd, filePath)
		...
		for nodeModule, binaries := range nodeModulesBinariesMap {
			if len(binaries) > 0 {
				...
				for ib := 0; ib < len(binaries) && !isUsed; ib++ {
					binary := binaries[ib]
					// Compiled once per binary, outside the loops over scripts and files.
					matcher := binaryWordMatcher(binary)

					if hasScripts && len(scripts) > 0 {
						for _, script := range scripts {
							// Cheap substring pre-filter: the regexp only runs when the name is
							// present at all, which is rare across a whole scripts block.
							if strings.Contains(script, binary) && matcher.MatchString(script) {
					...
					if !isUsed {
						for filePath, content := range additionalContentToLookUpForBinaries {
							if strings.Contains(content, binary) && matcher.MatchString(content) {
	...
	tsconfigPath := pathutil.JoinWithCwd(cwd, "tsconfig.json")
	if tsconfigJson != "" {
	...
	if merged, parseErr := resolve.ParseTsConfig(tsconfigPath); parseErr == nil {
...
func GetUnusedNodeModulesFromTree(
	...
	for moduleName := range cwdNodeModules {
		...
		if !has && !hasTypes && shouldIncludeModule(moduleName) {
			unused = append(unused, module.GetNodeModuleName(moduleName))
		}
	}
...
func GetMissingNodeModulesFromTree(
	...
	extraAllowedModules map[string]bool,
	nearestPackage bool,
	...
	for filePath, fileDeps := range minimalTree {
		for _, dependency := range fileDeps {
			if nearestPackage {
				// nearest-package mode: each import was resolved against the package.json that
				// owns its file, so NotResolvedModule already means "not declared by the owning
				// package". Report those directly and ignore the entry package.json entirely.
				// A module declared by another workspace package (and thus classified as
				// NodeModule for the file that owns it) is correctly NOT reported here.
				if dependency.ResolvedType == NotResolvedModule {
					moduleName := module.GetNodeModuleName(dependency.Request)
					if _, allowed := extraAllowedModules[moduleName]; allowed {
						continue
					}
			...
			// entry-package mode: files in the minimal tree might not belong to the entry when
			// following monorepo packages. During resolution, node modules are looked up by the
			// package.json that owns the file location, so we re-validate every node-module-ish
			// import against the entry package.json. This captures modules declared in e.g.
			// `shared` package.json, used by files from `shared`, but bundled by the entry app.
			if dependency.ResolvedType == NotResolvedModule || dependency.ResolvedType == NodeModule {
				moduleName := module.GetNodeModuleName(dependency.Request)
				_, declared := workingDirNodeModules[moduleName]
				_, allowed := extraAllowedModules[moduleName]
				if !declared && !allowed {
					setFilePathInNodeModuleFilesMap(&unresolved, moduleName, filePath)
				}
			}
	...
	for nodeModule, importedFromFiles := range unresolved {
		if shouldIncludeModule(nodeModule) && module.IsValidNodeModuleName(nodeModule) {
...
func setFilePathInNodeModuleFilesMap(nodeModuleFilesMap *map[string]map[string]bool, moduleName string, filePath string) {
	...
	key := pathutil.NormalizePathForInternal(filePath)
...
func formatUsedNodeModulesResult(
	...
	for depName := range usedNodeModules {
		if shouldIncludeModule(depName) && module.IsValidNodeModuleName(depName) {
			usedNodeModulesArr = append(usedNodeModulesArr, module.GetNodeModuleName(depName))
		}
	}
...
func FormatMissingNodeModulesResults(results []MissingNodeModuleResult, cwd string, countFlag bool, groupByModule bool, groupByFile bool, groupByModuleFilesCount bool) (string, int) {
	...
	cwdInternal := pathutil.NormalizePathForInternal(cwd)
...
func getGroupByFileResult(modulesArr []string, modulesFilesMap map[string]map[string]bool, cwd string) string {
	...
	usedByFileSorted := source.GetSortedMap(moduleByFile)
...
func getNodeModulesByEntryPoint(
	...
	for _, entryPointRaw := range absolutePathToEntryPoints {
		entryPoint := pathutil.NormalizePathForInternal(entryPointRaw)
	...
	graph := graph.BuildDepsGraphForMultiple(minimalTree, entryPoints, nil, false, false)

	moduleToFiles := map[string]map[string]bool{}
	uniqueFiles := map[string]bool{}
	for moduleName, filesSet := range usedNodeModules {
		if !shouldIncludeModule(moduleName) || !module.IsValidNodeModuleName(moduleName) {
			continue
		}
